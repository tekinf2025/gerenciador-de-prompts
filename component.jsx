...
LocalStorage do navegador (através do hook useStoredState do Hatch)
Não utiliza banco de dados tradicional - todos os dados são persistidos localmente
APIs Externas:
brapi.dev API para cotações da B3 (Bolsa de Valores)`,
    ...
    {
      id: 'saldos-bancarios',
      title: 'A sessão "Saldos Bancários" a edição dos valores está ruim',
      description: 'pode melhorar significativamente a experiência de digitação dos valores',
      category: 'Programação',
      model: 'LOVABLE',
      content: 'Melhorias na interface de edição de saldos bancários para melhor UX.',
      createdAt: new Date(Date.now() - 3 * 24 * 60 * 60 * 1000).toISOString(),
      liked: true,
      author: 'LOVABLE'
    },
    {
      id: 'analise-codigo',
      title: 'Você é analista de código profissional',
      description: 'Analise este projeto e verifique opções de melhorias e correções de erros https://hatchcanvas.com/',
      category: 'Projetos',
      model: 'HATCH.CANVAS',
      content: 'Prompt para análise profissional de código com foco em melhorias e correções.',
      createdAt: new Date(Date.now() - 3 * 24 * 60 * 60 * 1000).toISOString(),
      liked: false,
      author: 'HATCH.CANVAS'
    },
    {
      ...
      category: 'Projetos',
      model: 'LOVABLE',
      content: 'Sistema de controle financeiro versão 7.20 com funcionalidades avançadas.',
    ...
    },
    {
      ...
      description: 'Conteúdos errado ou faltando',
      category: 'Whatsapp',
      model: 'CHATGPT',
      content: 'Diagnóstico e correção de erros de servidor com conteúdos faltantes.',
      createdAt: new Date(Date.now() - 5 * 24 * 60 * 60 * 1000).toISOString(),
      liked: false,
      author: 'CHATGPT'
    },
    {
      ...
      category: 'Projetos',
      model: 'LOVABLE',
      content: 'Sistema de gestão de recebíveis de cartão de crédito.',
      createdAt: new Date(Date.now() - 5 * 24 * 60 * 60 * 1000).toISOString(),
      liked: true,
      author: 'LOVABLE'
    },
    {
      ...
      category: 'Projetos',
      model: 'LOVABLE',
      content: 'Sistema completo de vendas versão 6 com funcionalidades avançadas.',
      createdAt: new Date(Date.now() - 5 * 24 * 60 * 60 * 1000).toISOString(),
      liked: true,
      author: 'LOVABLE'
    },
    {
      ...
      category: 'Programação',
      model: 'SUPABASE',
      content: 'Configuração de políticas de acesso no Supabase para liberação de banco de dados.',
      createdAt: new Date(Date.now() - 5 * 24 * 60 * 60 * 1000).toISOString(),
      liked: false,
      author: 'SUPABASE'
    }
  ...
  const [categories, setCategories] = useStoredState('tekinformatica_categories', [
    { id: 'analise-dados', name: 'Análise de Dados', color: 'red', bgColor: 'bg-red-500/20', textColor: 'text-red-400', borderColor: 'border-red-500/50' },
    { id: 'educacao', name: 'Educação', color: 'blue', bgColor: 'bg-blue-500/20', textColor: 'text-blue-400', borderColor: 'border-blue-500/50' },
    ...
    { id: 'programacao', name: 'Programação', color: 'cyan', bgColor: 'bg-cyan-500/20', textColor: 'text-cyan-400', borderColor: 'border-cyan-500/50' },
  ...
  // Supabase Integration - Implementação corrigida
    ...
    try {
      ...
      switch (operation) {
        ...
        case 'update':
          ...
          const { error: updateError } = await client.from('categories')
            .update(categoryData)
            .eq('id', category.id)
            .eq('user_id', 'user_default');
          if (updateError) {
        ...
        case 'delete':
          console.log('🗑️ Deletando categoria do Supabase:', category.id);
          const { error: deleteError } = await client.from('categories')
            .delete()
            .eq('id', category.id)
            .eq('user_id', 'user_default');
          if (deleteError) {
      ...
      console.error('💥 Erro na sincronização de categoria:', error);
    }
  };
  ...
  const syncModelToSupabase = async (model, operation) => {
    ...
    try {
      ...
      switch (operation) {
        ...
        case 'delete':
          console.log('🗑️ Deletando modelo do Supabase:', model.id);
          const { error: deleteError } = await client.from('models')
            .delete()
            .eq('id', model.id)
            .eq('user_id', 'user_default');
          if (deleteError) {
      ...
      console.error('💥 Erro na sincronização de modelo:', error);
    }
  };

  // Função para sincronizar uma operação específica
    ...
    try {
      ...
      switch (operation) {
        ...
        case 'update':
          ...
          const { error: updateError } = await client.from('prompts')
            .update(promptData)
            .eq('id', prompt.id)
            .eq('user_id', 'user_default');
          if (updateError) {
        ...
        case 'delete':
          console.log('🗑️ Deletando prompt do Supabase:', prompt.id);
          const { error: deleteError } = await client.from('prompts')
            .delete()
            .eq('id', prompt.id)
            .eq('user_id', 'user_default');
          if (deleteError) {
  ...
  const syncToSupabase = async () => {
    ...
    try {
      ...
      // Sync categories with upsert logic
      ...
      // Sync models with upsert logic
      console.log('🤖 Sincronizando modelos...');
      const modelsData = models.map(model => ({
        id: model.id,
        name: model.name,
        icon_name: model.iconName || 'Code', // Salvar iconName no banco
        color: model.color,
        bg_color: model.bgColor,
        text_color: model.textColor,
        border_color: model.borderColor,
      ...
      // Sync prompts with upsert logic
      console.log('📝 Sincronizando prompts...');
      ...
      console.log('🎉 Sincronização para Supabase concluída com sucesso!');
      setLastSync(new Date().toISOString());
  ...
  const handleSupabaseConfig = async () => {
    ...
    if (isConnected) {
      ...
      alert('Falha na conexão com o Supabase. Verifique a URL e a chave de API.');
    }
  };
  ...
  useEffect(() => {
    if (supabaseConfig) {
      console.log('🔄 Testando conexão automática com Supabase...');
      testSupabaseConnection(supabaseConfig.url, supabaseConfig.key).then((connected) => {
        setSupabaseConnected(connected);
        if (connected) {
          console.log('✅ Conexão automática com Supabase estabelecida!');
  ...
  const getModelIcon = (modelName) => {
    ...
    // Mapear nomes de ícones para componentes React
    const iconMap = {
      'Zap': Zap,
      'Palette': Palette,
  ...
  const handleLikePrompt = async (promptId) => {
    ...
    // Atualizar o estado local primeiro (UX imediata)
    setPrompts(prev => prev.map(p => 
      p.id === promptId ? updatedPrompt : p
    ));
    
    // Sincronizar automaticamente com o Supabase se habilitado
    if (supabaseConnected && autoSync) {
      console.log('🔄 Sincronizando like/dislike automaticamente...');
  ...
  const handleAddPrompt = async () => {
    if (newPrompt.title && newPrompt.content) {
      ...
      // Sincronizar automaticamente com o Supabase se habilitado
      if (supabaseConnected && autoSync) {
        console.log('🔄 Sincronizando novo prompt automaticamente...');
  ...
  const handleUpdatePrompt = async () => {
    if (editingPrompt && newPrompt.title && newPrompt.content) {
      ...
      // Sincronizar automaticamente com o Supabase se habilitado
      if (supabaseConnected && autoSync) {
        console.log('🔄 Sincronizando prompt editado automaticamente...');
        await syncPromptToSupabase(updatedPrompt, 'update');
      }
    }
  };
  ...
  const handleDeletePrompt = async (promptId) => {
    ...
    // Sincronizar automaticamente com o Supabase se habilitado
    if (supabaseConnected && autoSync) {
      console.log('🔄 Deletando prompt do Supabase automaticamente...');
  ...
  const handleAddCategory = async () => {
    if (newCategory.trim()) {
      if (editingCategory) {
        ...
        if (supabaseConnected && autoSync) {
          console.log('🔄 Sincronizando categoria editada automaticamente...');
        ...
        // Sincronizar automaticamente se habilitado
        if (supabaseConnected && autoSync) {
          console.log('🔄 Sincronizando nova categoria automaticamente...');
  ...
  const handleAddModel = async () => {
    if (newModel.trim()) {
      if (editingModel) {
        ...
        // Sincronizar automaticamente se habilitado
        if (supabaseConnected && autoSync) {
          console.log('🔄 Sincronizando modelo editado automaticamente...');
        ...
        const model = {
          ...
          iconName: 'Code', // Usar iconName ao invés de icon
          color: randomColor,
          bgColor: `bg-${randomColor}-500/20`,
          textColor: `text-${randomColor}-400`,
          borderColor: `border-${randomColor}-500/50`
        ...
        // Sincronizar automaticamente se habilitado
        if (supabaseConnected && autoSync) {
          console.log('🔄 Sincronizando novo modelo automaticamente...');
  ...
  const handleDeleteCategory = async (categoryId) => {
    if (window.confirm('Tem certeza que deseja excluir esta categoria?')) {
      ...
      // Sincronizar automaticamente se habilitado
      if (supabaseConnected && autoSync) {
        console.log('🔄 Deletando categoria do Supabase automaticamente...');
  ...
  const handleDeleteModel = async (modelId) => {
    if (window.confirm('Tem certeza que deseja excluir este modelo?')) {
      ...
      // Sincronizar automaticamente se habilitado
      if (supabaseConnected && autoSync) {
        console.log('🔄 Deletando modelo do Supabase automaticamente...');
  ...
  const PromptCard = ({ prompt }) => (
    ...
    >
      <div className="flex items-start justify-between mb-3 flex-1">
        <div className="flex-1 pr-4 flex flex-col h-full">
          <div className="flex items-center justify-between mb-3">
            ...
            {/* Botões do topo - Favorito e Menu */}
            <div className="flex items-center gap-2">
              ...
              {/* Dropdown Menu - Versão Mobile First */}
      ...
      {/* Botões de ação principais - Alinhados à direita */}
  ...
  return (
    ...
    >
            ...
            <div className="flex items-center gap-4 mb-2">
                  ...
                  <div className="flex items-center gap-1">
                    {autoSync ? (
                      <>
                        ...
                        <span className="text-xs">Sincronização automática</span>
                      </>
                    ) : (
                      <>
                        <div className="w-2 h-2 bg-yellow-400 rounded-full"></div>
                        <span className="text-xs">Sincronização manual</span>
                      </>
                    )}
                  </div>
                  {lastSync && (
              ...
              {syncStatus === 'error' && (
                <div className="bg-red-600 text-white px-3 py-1 rounded text-sm inline-block">
                  Erro na sincronização - Verifique o console
                </div>
              )}
            </div>
          </div>
      ...
      {showSupabaseConfig && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-gray-800 rounded-lg p-6 w-full max-w-md">
            <h3 className="text-lg font-semibold mb-4">Configurar Supabase</h3>
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium mb-1">Chave de API (anon/public)</label>
                <input
                  type="password"
                  placeholder="eyJ..."
              ...
              <div className="text-xs text-gray-400 bg-gray-700/50 p-3 rounded">
                <p><strong>Configuração Automática:</strong></p>
                <p>✅ URL: https://pomzjemdbaawksttbmgj.supabase.co</p>
                <p>✅ Chave: Configurada automaticamente</p>
                <p>💡 A conexão será estabelecida automaticamente</p>
              </div>
            </div>
